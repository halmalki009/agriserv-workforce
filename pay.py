...
# ملف العمل الإضافي مستقل. يقرأ أحدث ملف بيانات العمل الإضافي سواء داخل data أو بجانب pay.py
_work_extra_candidates = []
if DATA_DIR.exists():
    _work_extra_candidates.extend(list(DATA_DIR.glob("Overtime*.xlsx")))
    _work_extra_candidates.extend(list(DATA_DIR.glob("overtime*.xlsx")))
_work_extra_candidates.extend(list(APP_DIR.glob("Overtime*.xlsx")))
_work_extra_candidates.extend(list(APP_DIR.glob("overtime*.xlsx")))
_work_extra_candidates = sorted(set(_work_extra_candidates), key=lambda p: p.stat().st_mtime, reverse=True)
OVERTIME_PATH = _work_extra_candidates[0] if _work_extra_candidates else None

# ملف رحلات العمل والتذاكر مستقل. يقرأ أحدث ملف مطابق من data أو من نفس مجلد pay.py
_business_trip_candidates = []
for pattern in [
    "مصروف الانتداب والتذاكر*.xlsx",
    "business_trip*.xlsx",
    "business_trips*.xlsx",
    "trips*.xlsx",
]:
    _business_trip_candidates.extend(list(DATA_DIR.glob(pattern)) if DATA_DIR.exists() else [])
    _business_trip_candidates.extend(list(APP_DIR.glob(pattern)))

_business_trip_candidates = sorted(set(_business_trip_candidates), key=lambda p: p.stat().st_mtime, reverse=True)
BUSINESS_TRIPS_PATH = _business_trip_candidates[0] if _business_trip_candidates else None
...
def load_work_extra_data():
    ...
    raw.columns = [
        re.sub(r'\s+', ' ', str(c).replace('\u200f', '').replace('\u200e', '').replace('\xa0', ' ')).strip()
    ...
    required_text_cols = ['القطاع', 'الإدارة العامة', 'الإدارة', 'نوع الموظف', 'كادر الموظف', 'الموقع الفعلي']
    ...
    dimension_cols = ['م', 'الرقم الوظيفي', 'اسم الموظف', 'نوع الموظف', 'كادر الموظف', 'القطاع', 'الإدارة العامة', 'الإدارة', 'الموقع الفعلي', 'الإجمالي الكلي']
    ...
    long_df = raw.melt(
        id_vars=[c for c in ['الرقم الوظيفي', 'اسم الموظف', 'نوع الموظف', 'كادر الموظف', 'القطاع', 'الإدارة العامة', 'الإدارة', 'الموقع الفعلي'] if c in raw.columns],
...
def load_business_trips_data():
    ...
    if 'حالة الموظف' in raw.columns:
        raw['حالة الموظف'] = clean_text_value(raw['حالة الموظف']).replace('', 'غير محدد')
    else:
...
def apply_business_trip_global_filters(trips_df: pd.DataFrame) -> pd.DataFrame:
    """تطبيق الفلاتر العامة على رحلات العمل دون تطبيق أي فلتر شهري عام."""
    result = trips_df.copy()

    if result.empty:
        return result

    if type_choice != "الكل" and 'نوع الموظف' in result.columns:
        selected_type = normalize_employee_type_value(type_choice)
        result = result[result['نوع الموظف'].apply(normalize_employee_type_value).eq(selected_type)]

    if sector_choice and 'القطاع' in result.columns:
        result = result[result['القطاع'].isin(sector_choice)]

    if general_admin_choice and 'الإدارة العامة' in result.columns:
        result = result[result['الإدارة العامة'].isin(general_admin_choice)]

    if admin_choice and 'الإدارة' in result.columns:
        result = result[result['الإدارة'].isin(admin_choice)]

    if location_choice and 'الموقع الفعلي' in result.columns:
        result = result[result['الموقع الفعلي'].isin(location_choice)]
...
def prepare_workforce_cost_view(base_df: pd.DataFrame, work_extra_long_df: pd.DataFrame, business_trips_df: pd.DataFrame, company_cost_mode: str) -> pd.DataFrame:
    ...
    if work_extra_long_df is not None and not work_extra_long_df.empty and '_person_id_int' in result.columns:
        ...
        if not ot.empty:
            ...
            if company_cost_mode == "التكلفة الشاملة":
                ...
                inactive_ot = ot_by_emp[~ot_by_emp['الرقم الوظيفي'].dropna().astype(int).isin(existing_ids)].copy()

                if not inactive_ot.empty:
                    inactive_rows = pd.DataFrame(columns=result.columns)
                    inactive_rows['معرّف الشخص'] = inactive_ot['الرقم الوظيفي'].astype('Int64')
                    inactive_rows['القطاع'] = clean_text_value(inactive_ot['القطاع'])
                    inactive_rows['الإدارة العامة'] = clean_text_value(inactive_ot['الإدارة_العامة_أوفر_تايم'])
                    inactive_rows['الإدارة'] = clean_text_value(inactive_ot['الإدارة_أوفر_تايم'])
                    inactive_rows['نوع الموظف'] = inactive_ot['نوع_الموظف_أوفر_تايم'].apply(normalize_employee_type_value) if 'نوع_الموظف_أوفر_تايم' in inactive_ot.columns else 'غير نشط'
                    inactive_rows['الدرجة الوظيفية'] = ''
                    inactive_rows['مكان العمل'] = ''
                    inactive_rows['الموقع الفعلي'] = clean_text_value(inactive_ot['الموقع_الفعلي_أوفر_تايم']) if 'الموقع_الفعلي_أوفر_تايم' in inactive_ot.columns else ''
                    inactive_rows['تكلفة الشركة الشهرية الأساسية'] = 0.0
                    inactive_rows['تكلفة الشركة السنوية الأساسية'] = 0.0
                    inactive_rows['تكاليف إضافية سنوية - عمل إضافي'] = inactive_ot['إجمالي_الأوفر_تايم'].fillna(0).values
                    inactive_rows['تكاليف إضافية شهرية - عمل إضافي'] = inactive_rows['تكاليف إضافية سنوية - عمل إضافي'] / 12
                    inactive_rows['تكلفة الوزارة الشهرية'] = 0.0
                    inactive_rows['تكلفة الوزارة السنوية'] = 0.0
                    result = pd.concat([result, inactive_rows], ignore_index=True, sort=False)
    ...
    if business_trips_df is not None and not business_trips_df.empty and '_person_id_int' in result.columns:
        ...
        if not trips.empty:
            ...
            if company_cost_mode == "التكلفة الشاملة":
                existing_ids = set(result['_person_id_int'].dropna().astype(int).tolist())
                inactive_trips = trips_by_emp[~trips_by_emp['الرقم الوظيفي'].dropna().astype(int).isin(existing_ids)].copy()

                if not inactive_trips.empty:
                    inactive_rows = pd.DataFrame(columns=result.columns)
                    inactive_rows['معرّف الشخص'] = inactive_trips['الرقم الوظيفي'].astype('Int64')
                    inactive_rows['القطاع'] = clean_text_value(inactive_trips['القطاع'])
                    inactive_rows['الإدارة العامة'] = clean_text_value(inactive_trips['الإدارة_العامة_رحلات'])
                    inactive_rows['الإدارة'] = clean_text_value(inactive_trips['الإدارة_رحلات'])
                    inactive_rows['نوع الموظف'] = inactive_trips['نوع_الموظف_رحلات'].apply(normalize_employee_type_value)
                    inactive_rows['الدرجة الوظيفية'] = ''
                    inactive_rows['مكان العمل'] = ''
                    inactive_rows['الموقع الفعلي'] = clean_text_value(inactive_trips['الموقع_الفعلي_رحلات'])
                    inactive_rows['تكلفة الشركة الشهرية الأساسية'] = 0.0
                    inactive_rows['تكلفة الشركة السنوية الأساسية'] = 0.0
                    inactive_rows['تكاليف إضافية سنوية - رحلات العمل'] = inactive_trips['إجمالي_رحلات_العمل'].fillna(0).values
                    inactive_rows['تكاليف إضافية شهرية - رحلات العمل'] = inactive_rows['تكاليف إضافية سنوية - رحلات العمل'] / 12
                    inactive_rows['تكلفة الوزارة الشهرية'] = 0.0
                    inactive_rows['تكلفة الوزارة السنوية'] = 0.0
                    result = pd.concat([result, inactive_rows], ignore_index=True, sort=False)
...
def apply_work_extra_filters(work_extra_long_df: pd.DataFrame) -> pd.DataFrame:
    """تطبيق فلاتر اللوحة العامة على ملف العمل الإضافي دون ربط إلزامي ببيانات الموظفين الحالية."""
    result = work_extra_long_df.copy()
    ...
    if type_choice != "الكل":
        selected_type = normalize_employee_type_value(type_choice)
        if 'كادر الموظف' in result.columns:
            result = result[result['كادر الموظف'].apply(normalize_employee_type_value).eq(selected_type)]
        elif 'نوع الموظف' in result.columns:
            result = result[result['نوع الموظف'].apply(normalize_employee_type_value).eq(selected_type)]
...
def render_work_extra_tab(work_extra_raw_df: pd.DataFrame, work_extra_long_df: pd.DataFrame, current_employee_df: pd.DataFrame):
    ...
    current_ids = set(pd.to_numeric(current_employee_df.get('معرّف الشخص', pd.Series(dtype='float')), errors='coerce').dropna().astype(int).tolist())
    ot_base = ot_base.copy()
    ot_base['حالة الموظف'] = ot_base['الرقم الوظيفي'].astype(int).apply(
        lambda x: 'نشط' if x in current_ids else 'غير نشط'
    )
    ...
    with chart_cols[0]:
        fig_month = px.bar(
            month_summary,
            x='الشهر / المناسبة',
            y='إجمالي_الأوفر_تايم',
            text='عدد_الموظفين',
            title="توزيع العمل الإضافي حسب الشهر / المناسبة"
        )
        fig_month.update_layout(xaxis_title="", yaxis_title="إجمالي العمل الإضافي", height=420)
    ...
    with chart_cols[1]:
        fig_entity = px.bar(
            ...
            y='إجمالي_الأوفر_تايم',
            text='عدد_الموظفين',
            title="أعلى الجهات حسب العمل الإضافي"
        )
        fig_entity.update_layout(xaxis_title="", yaxis_title="إجمالي العمل الإضافي", height=420)
        fig_entity.update_yaxes(tickformat=',.0f')
        render_chart(fig_entity)
    ...
    fig_admin = px.bar(
        ...
        y='إجمالي_الأوفر_تايم',
        text='عدد_الموظفين',
        title="أعلى 15 إدارة حسب العمل الإضافي"
    )
    fig_admin.update_layout(xaxis_title="", yaxis_title="إجمالي العمل الإضافي", height=460)
    fig_admin.update_yaxes(tickformat=',.0f')
    render_chart(fig_admin)

    summary_tabs = st.tabs(["حسب الجهة", "حسب القطاع", "حسب الإدارة العامة", "حسب الإدارة", "أعلى الموظفين", "التفاصيل الشهرية"])

    with summary_tabs[0]:
        st.dataframe(format_financial_dataframe(entity_ot), use_container_width=True, hide_index=True)

    with summary_tabs[1]:
        st.dataframe(format_financial_dataframe(sector_ot), use_container_width=True, hide_index=True)

    with summary_tabs[2]:
        st.dataframe(format_financial_dataframe(general_admin_ot), use_container_width=True, hide_index=True)

    with summary_tabs[3]:
        st.dataframe(format_financial_dataframe(admin_ot), use_container_width=True, hide_index=True)

    with summary_tabs[4]:
        st.dataframe(format_financial_dataframe(employee_ot.head(50)), use_container_width=True, hide_index=True)
    ...
    with summary_tabs[5]:
        ...
        # الفرز يتم قبل اختيار أعمدة العرض حتى لا يتم حذف عمود ترتيب الشهر الداخلي
        sort_cols = [c for c in ['_ترتيب_الشهر', 'مبلغ العمل الإضافي'] if c in ot_filtered.columns]
        if sort_cols:
            details_source = ot_filtered.sort_values(sort_cols, ascending=[True, False][:len(sort_cols)]).copy()
        else:
            details_source = ot_filtered.copy()
        details = details_source[detail_cols].copy()
        st.dataframe(format_financial_dataframe(details), use_container_width=True, hide_index=True)
...
def render_business_trips_tab(trips_df: pd.DataFrame, current_employee_df: pd.DataFrame):
    ...
    current_ids = set(pd.to_numeric(current_employee_df.get('معرّف الشخص', pd.Series(dtype='float')), errors='coerce').dropna().astype(int).tolist())
    base = base.copy()
    base['حالة الموظف'] = base['الرقم الوظيفي'].astype(int).apply(lambda x: 'نشط' if x in current_ids else 'غير نشط')
    ...
    with chart_cols[1]:
        fig_components = px.bar(
            month_summary,
            x='الشهر',
            y=['بدل_الانتداب', 'التذاكر'],
            title="بدل الانتداب مقابل التذاكر حسب الشهر",
            barmode='group'
        )
        fig_components.update_layout(xaxis_title="", yaxis_title="القيمة", height=420)
        fig_components.update_yaxes(tickformat=',.0f')
        render_chart(fig_components)
    ...
    fig_entity = px.bar(
        ...
        y='إجمالي_رحلات_العمل',
        text='عدد_الرحلات',
        title="أعلى الجهات حسب تكلفة رحلات العمل"
    )
    fig_entity.update_layout(xaxis_title="", yaxis_title="إجمالي رحلات العمل", height=460)
    fig_entity.update_yaxes(tickformat=',.0f')
    render_chart(fig_entity)

    summary_tabs = st.tabs([
        "حسب الجهة", "حسب القطاع", "حسب الإدارة العامة", "حسب الإدارة",
        "حسب الموقع", "حسب المدينة", "حسب سبب الرحلة", "أعلى الموظفين", "التفاصيل"
    ])

    with summary_tabs[0]:
        st.dataframe(format_financial_dataframe(entity_summary), use_container_width=True, hide_index=True)
    with summary_tabs[1]:
        st.dataframe(format_financial_dataframe(sector_summary), use_container_width=True, hide_index=True)
    with summary_tabs[2]:
        st.dataframe(format_financial_dataframe(general_admin_summary), use_container_width=True, hide_index=True)
    with summary_tabs[3]:
        st.dataframe(format_financial_dataframe(admin_summary), use_container_width=True, hide_index=True)
    with summary_tabs[4]:
        st.dataframe(format_financial_dataframe(location_summary), use_container_width=True, hide_index=True)
    with summary_tabs[5]:
        st.dataframe(format_financial_dataframe(city_summary), use_container_width=True, hide_index=True)
    with summary_tabs[6]:
        st.dataframe(format_financial_dataframe(reason_summary), use_container_width=True, hide_index=True)
    with summary_tabs[7]:
        st.dataframe(format_financial_dataframe(employee_summary.head(50)), use_container_width=True, hide_index=True)
    with summary_tabs[8]:
        detail_cols = [
            'الرقم الوظيفي', 'نوع الموظف', 'حالة الموظف', 'القطاع', 'الإدارة العامة', 'الإدارة',
        ...
        st.dataframe(format_financial_dataframe(details[detail_cols]), use_container_width=True, hide_index=True)
...
if type_choice != "الكل":
    filtered = filtered[filtered['نوع الموظف'].apply(normalize_employee_type_value) == normalize_employee_type_value(type_choice)]
...
<div style="
    ...
    text-align:right;
">
📍 تحليل عام: القطاع ← الإدارة العامة ← الإدارة ← الموقع الفعلي ← الموظف
</div>
""", unsafe_allow_html=True)
...
if type_choice != "الكل":
    ...
    st.caption("للتفصيل حسب الجهات والإدارات والمواقع والموظفين استخدم تبويب تحليل عام.")
...
with tab0:
    ...
    st.markdown("### تحليل عام حسب الاختيار الحالي")
    st.caption("استعرض تكلفة القوى العاملة عبر المستويات التنظيمية بدءاً من القطاع وحتى الموظف.")
...
with tab6:
    ...
    # سجل الحالات المعالجة محفوظ في ملف جانبي ولا يغيّر الداتا الرئيسية
    ...
    alerts_df['إجراء المعالجة'] = alerts_df['_alert_key'].map(
        lambda key: alerts_status_data.get(key, {}).get('status', '') if isinstance(alerts_status_data.get(key, {}), dict) else ''
    )
    alerts_df['تاريخ المعالجة'] = alerts_df['_alert_key'].map(
        lambda key: alerts_status_data.get(key, {}).get('updated_at', '') if isinstance(alerts_status_data.get(key, {}), dict) else ''
    )

    open_alerts_df = alerts_df[~alerts_df['تمت المعالجة']].copy()
    processed_alerts_df = alerts_df[alerts_df['تمت المعالجة']].copy()

    critical_df = open_alerts_df[open_alerts_df['حالة الاستحقاق'].eq('حرج')].copy()
    warning_df = open_alerts_df[open_alerts_df['حالة الاستحقاق'].eq('تنبيه')].copy()
    stable_df = open_alerts_df[open_alerts_df['حالة الاستحقاق'].eq('مستقر')].copy()

    affected_df = open_alerts_df[open_alerts_df['حالة الاستحقاق'].isin(['حرج', 'تنبيه'])].copy()
    affected_cost = float(affected_df[monthly_col].sum()) if not affected_df.empty else 0

    nearest_days = open_alerts_df['الأيام المتبقية'].dropna().min()
    ...
    show_processed_alerts = st.checkbox(
        "عرض الحالات المعالجة",
        value=False,
        key="show_processed_alerts"
    )

    alert_view_df = processed_alerts_df.copy() if show_processed_alerts else open_alerts_df.copy()
    ...
    processed_extra_cols = ['إجراء المعالجة', 'تاريخ المعالجة'] if show_processed_alerts else []
    alert_display_cols = [c for c in base_alert_display_cols + processed_extra_cols if c in alert_view_df.columns]

    if alert_view_df.empty:
        if show_processed_alerts:
            st.info("لا توجد حالات معالجة مطابقة للتصفية الحالية.")
        else:
            st.info("لا توجد حالات مفتوحة مطابقة للتصفية الحالية.")
    else:
        if show_processed_alerts:
            select_all_processed = st.checkbox("تحديد الكل للمعروض", value=False, key="select_all_processed_alerts")
            editor_df = alert_view_df[alert_display_cols + ['_alert_key']].copy()
            editor_df.insert(0, 'إلغاء الإجراء', bool(select_all_processed))
            ...
            edited_alerts_df = st.data_editor(
                ...
                use_container_width=True,
                hide_index=True,
                disabled=[c for c in editor_df.columns if c != 'إلغاء الإجراء'],
                column_config={
                    '_alert_key': None,
                    'إلغاء الإجراء': st.column_config.CheckboxColumn('إلغاء الإجراء', help='حدد الحالات التي تريد إعادتها إلى التنبيهات المفتوحة')
                },
                key='processed_alerts_editor'
            )

            selected_cancel_keys = edited_alerts_df.loc[
                edited_alerts_df['إلغاء الإجراء'].eq(True), '_alert_key'
            ].dropna().astype(str).tolist()

            cancel_cols = st.columns([1, 5])
            with cancel_cols[0]:
                if st.button("↩ إلغاء الإجراء", use_container_width=True, disabled=not selected_cancel_keys, key="cancel_processed_alerts"):
                    current_status = load_alerts_status()
                    for key_value in selected_cancel_keys:
                        current_status.pop(key_value, None)
                    save_alerts_status(current_status)
                    st.success("تم إلغاء الإجراء وإعادة الحالات المحددة إلى التنبيهات المفتوحة.")
                    st.rerun()
        else:
            select_all_open = st.checkbox("تحديد الكل للمعروض", value=False, key="select_all_open_alerts")
            editor_df = alert_view_df[alert_display_cols + ['_alert_key']].copy()
            editor_df.insert(0, 'تحديد', bool(select_all_open))
            editor_df = format_financial_dataframe(editor_df)
            ...
            edited_alerts_df = st.data_editor(
                ...
                column_config={
                    '_alert_key': None,
                    'تحديد': st.column_config.CheckboxColumn('تحديد', help='حدد الحالات ثم اختر تم التجديد أو تم الإنهاء')
                },
                key='open_alerts_editor'
            )

            selected_alert_keys = edited_alerts_df.loc[
                edited_alerts_df['تحديد'].eq(True), '_alert_key'
            ].dropna().astype(str).tolist()

            action_cols = st.columns([1, 1, 4])
            with action_cols[0]:
                if st.button("تم التجديد", use_container_width=True, disabled=not selected_alert_keys, key="renew_selected_alerts"):
                    current_status = load_alerts_status()
                    now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    for key_value in selected_alert_keys:
                    ...
                    st.rerun()
            with action_cols[1]:
                if st.button("تم الإنهاء", use_container_width=True, disabled=not selected_alert_keys, key="end_selected_alerts"):
                    current_status = load_alerts_status()
                    now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    for key_value in selected_alert_keys:
                        current_status[key_value] = {
    ...
    status_summary = open_alerts_df.groupby('مؤشر الاستحقاق', dropna=False).agg(
        عدد_الموظفين=('معرّف الشخص', 'count'),
    ...
    if show_alert_chart and not status_summary.empty:
        fig_alerts = px.bar(
            ...
            y='عدد_الموظفين',
            text='عدد_الموظفين',
            title="توزيع حالات الاستحقاق المفتوحة حسب عدد الموظفين"
        )
        fig_alerts.update_layout(xaxis_title="", yaxis_title="عدد الموظفين", height=380)
        render_chart(fig_alerts)

    download_df = alert_view_df[alert_display_cols].copy() if not alert_view_df.empty else pd.DataFrame(columns=alert_display_cols)
    st.download_button(
